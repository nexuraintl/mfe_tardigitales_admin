...
import { TarjetaSociedad } from '../../tarjetas-sociedades.component';

@Component({
  selector: 'app-historial-sociedades',
  standalone: true,
  imports: [CommonModule],
  template: `
    <ng-container *ngIf="selectedTarjeta">
      <!-- TARJETA SUPERIOR RESUMEN DE LA SOCIEDAD -->
        ...
        <div class="card-body p-4 d-flex align-items-center gap-3 flex-wrap">
          ...
          <div class="flex-grow-1 min-w-0">
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-2">
              <h5 class="fw-bold text-dark mb-0" style="font-size: 16px;">{{ selectedTarjeta.razon_social }}</h5>
              <div class="ms-auto d-flex align-items-center gap-2">
                ...
                <span class="badge bg-success-subtle text-success border border-success border-opacity-25 px-3 py-1.5 fw-bold" style="font-size: 11px;">
                  {{ selectedTarjeta.estado_tarjeta || 'Activa' }}
                </span>
              </div>
            </div>

            <div class="d-flex flex-wrap gap-3 text-secondary small" style="font-size: 12px;">
              <span>NIT: <strong class="text-dark">{{ selectedTarjeta.nit }}</strong></span>
              <span>No. Inscripción: <strong class="text-dark">{{ selectedTarjeta.inscripcion }}</strong></span>
              <span>Expediente: <strong class="text-dark">{{ selectedTarjeta.no_expd }}</strong></span>
              <span *ngIf="selectedTarjeta.representante_legal || selectedTarjeta.representante">Representante: <strong class="text-dark">{{ selectedTarjeta.representante_legal || selectedTarjeta.representante }}</strong></span>
...
export class HistorialSociedadesComponent {
  @Input() selectedTarjeta: TarjetaSociedad | null = null;
